...
<body>
								...
								<c:forEach items="${page.rows}" var="row">
									<tr>
										...
										<td style="text-align:center;"><a href="#" class="btn btn-primary btn-xs"
											...
											onclick="editStudent(${row.student_id})">修改</a> <a href="#"
											class="btn btn-danger btn-xs"
											onclick="deleteStudent(${row.student_id})">删除</a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
			...
			<div class="modal-content">
					...
					<form class="form-horizontal" id="new_student_form">
						<div class="form-group">
							<label for="new_studentName" class="col-sm-2 control-label">
								学生姓名 </label>
							<div class="col-sm-10">
								<input type="text" class="form-control" id="new_studentname"
									placeholder="学生姓名" name="student_name" />
							</div>
						</div>
						<div class="form-group">
							<label for="new_studenttie"
								style="float:left;padding:7px 15px 0 27px;">学生系别</label>
							<div class="col-sm-10">
								<select class="form-control" id="new_studenttie"
									name="student_tie">
									<option value="">--请选择--</option>
									<c:forEach items="${tieType}" var="item">
										<option value="${item.dict_id}"
											<c:if test="${item.dict_id == studenttie}">selected</c:if>>
											${item.dict_item_name }</option>
									</c:forEach>
								</select>
							</div>
						...
						<div class="form-group">
							<label for="new_studentmajor"
								style="float:left;padding:7px 15px 0 27px;">学生专业</label>
							<div class="col-sm-10">
								<select class="form-control" id="new_studentmajor"
									name="student_major">
									<option value="">--请选择--</option>
									<c:forEach items="${majorType}" var="item">
										<option value="${item.dict_id}"
											<c:if test="${item.dict_id == studentmajor}"> selected</c:if>>
											${item.dict_item_name }</option>
									</c:forEach>
								</select>
						...
						<div class="form-group">
							<label for="new_studentgrade"
								style="float:left;padding:7px 15px 0 27px;">学生年级</label>
							<div class="col-sm-10">
								<select class="form-control" id="new_studentgrade"
									name="student_grade">
									<option value="">--请选择--</option>
									<c:forEach items="${gradeType}" var="item">
										<option value="${item.dict_id}"
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="new_sex"
									placeholder="性别" name="student_sex" />
							</div>
						</div>
						<div class="form-group">
							<label for="new_nation" class="col-sm-2 control-label">民族</label>
							<div class="col-sm-10">
								<input type="text" class="form-control" id="new_nation"
									placeholder="民族" name="student_nation" />
							</div>
						</div>
						<div class="form-group">
							<label for="new_birth" class="col-sm-2 control-label">出生日期</label>
							<div class="col-sm-10">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="new_place"
									placeholder="籍贯" name="student_place" />
							</div>
						</div>
					</form>
				</div>
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary"
						onclick="createStudent()">新建</button>
				</div>
			</div>
		</div>
	</div>
	...
	//清空新建客户窗口中的数据
		function clearStudent() {
		    $("#new_studentName").val("");
		    $("#new_studentmajor").val("");
		    $("#new_studentgrade").val("");
		    $("#new_studenttie").val("");
		    $("#new_sex").val("");
		    $("#new_nation").val();
		    $("#new_birth").val("");
